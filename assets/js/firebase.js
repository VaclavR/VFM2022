import { initializeApp } from "firebase/app";
import { getDatabase, ref, get, set, child, query, onValue, orderByChild, equalTo } from "firebase/database";

const players = [
    {
      id: 1,
      name: "Tomáš Vaclík",
      teamId: 1,
      age: 33,
      position: "brankář",
      price: 20000000,
    },
    {
      id: 2,
      name: "David Lischka",
      teamId: 1,
      age: 26,
      position: "obránce",
      price: 5000000,
    },
    {
      id: 3,
      name: "Ondřej Čelůstka",
      teamId: 1,
      age: 32,
      position: "obránce",
      price: 15000000,
    },
    {
      id: 4,
      name: "Michal Šmíd",
      teamId: 1,
      age: 29,
      position: "obránce",
      price: 8000000,
    },
    {
      id: 5,
      name: "Filip Panák",
      teamId: 1,
      age: 24,
      position: "záložník",
      price: 10000000,
    },
    {
      id: 6,
      name: "Lukáš Štetina",
      teamId: 1,
      age: 26,
      position: "záložník",
      price: 7000000,
    },
    {
      id: 7,
      name: "Lukáš Juliš",
      teamId: 1,
      age: 28,
      position: "záložník",
      price: 12000000,
    },
    {
      id: 8,
      name: "Michal Trávník",
      teamId: 1,
      age: 28,
      position: "záložník",
      price: 10000000,
    },
    {
      id: 9,
      name: "Lukáš Juliš",
      teamId: 1,
      age: 28,
      position: "útočník",
      price: 12000000,
    },
    {
      id: 10,
      name: "Adam Hložek",
      teamId: 1,
      age: 18,
      position: "útočník",
      price: 35000000,
    },
    {
      id: 11,
      name: "Matěj Hanousek",
      teamId: 1,
      age: 24,
      position: "útočník",
      price: 9000000,
    },
    {
      id: 12,
      name: 'Ondřej Kolář',
      teamId: 2,
      age: 27,
      position: 'brankář',
      price: 5000000
    },
    {
      id: 13,
      name: 'Jan Bořil',
      teamId: 2,
      age: 30,
      position: 'obránce',
      price: 3000000
    },
    {
      id: 14,
      name: 'Ondřej Kúdela',
      teamId: 2,
      age: 34,
      position: 'obránce',
      price: 2000000
    },
    {
      id: 15,
      name: 'Simon Deli',
      teamId: 2,
      age: 29,
      position: 'obránce',
      price: 2500000
    },
    {
      id: 16,
      name: 'Jan Kuchta',
      teamId: 2,
      age: 26,
      position: 'záložník',
      price: 4000000
    },
    {
      id: 17,
      name: 'Ibrahim Traoré',
      teamId: 2,
      age: 24,
      position: 'záložník',
      price: 3500000
    },
    {
      id: 18,
      name: 'Nicolae Stanciu',
      teamId: 2,
      age: 28,
      position: 'záložník',
      price: 8000000
    },
    {
      id: 19,
      name: 'Péter Olayinka',
      teamId: 2,
      age: 26,
      position: 'útočník',
      price: 5000000
    },
    {
      id: 20,
      name: 'Abdallah Sima',
      teamId: 2,
      age: 20,
      position: 'útočník',
      price: 10000000
    },
    {
      id: 21,
      name: 'Jan Novák',
      teamId: 3,
      age: 24,
      position: 'obránce',
      price: 1500000
    },
    {
      id: 22,
      name: 'Petr Horák',
      teamId: 3,
      age: 28,
      position: 'obránce',
      price: 2000000
    },
    {
      id: 23,
      name: 'Tomáš Turek',
      teamId: 3,
      age: 26,
      position: 'obránce',
      price: 1800000
    },
    {
      id: 24,
      name: 'Lukáš Marek',
      teamId: 3,
      age: 22,
      position: 'záložník',
      price: 2500000
    },
    {
      id: 25,
      name: 'Michal Novotný',
      teamId: 3,
      age: 29,
      position: 'záložník',
      price: 3000000
    },
    {
      id: 26,
      name: 'Jakub Pospíšil',
      teamId: 3,
      age: 25,
      position: 'záložník',
      price: 2700000
    },
    {
      id: 27,
      name: 'Martin Vlk',
      teamId: 3,
      age: 27,
      position: 'záložník',
      price: 2800000
    },
    {
      id: 28,
      name: 'Pavel Hora',
      teamId: 3,
      age: 23,
      position: 'útočník',
      price: 3500000
    },
    {
      id: 29,
      name: 'David Štěpán',
      teamId: 3,
      age: 30,
      position: 'útočník',
      price: 4000000
    },
    {
      id: 30,
      name: 'Marek Procházka',
      teamId: 3,
      age: 28,
      position: 'útočník',
      price: 4200000
    },
    {
      id: 31,
      name: 'Jan Král',
      teamId: 3,
      age: 27,
      position: 'brankář',
      price: 5000000
    },
      {
        id: 32,
        name: "Jan Novák",
        teamId: 4,
        age: 25,
        position: "brankář",
        price: 50000
      },
      {
        id: 33,
        name: "Pavel Vávra",
        teamId: 4,
        age: 28,
        position: "obránce",
        price: 20000
      },
      {
        id: 34,
        name: "Tomáš Hájek",
        teamId: 4,
        age: 24,
        position: "obránce",
        price: 15000
      },
      {
        id: 35,
        name: "David Limberský",
        teamId: 4,
        age: 37,
        position: "obránce",
        price: 25000
      },
      {
        id: 36,
        name: "Petr Ševčík",
        teamId: 4,
        age: 26,
        position: "záložník",
        price: 30000
      },
      {
        id: 37,
        name: "Michal Kovařík",
        teamId: 4,
        age: 32,
        position: "záložník",
        price: 35000
      },
      {
        id: 38,
        name: "Jan Kopic",
        teamId: 4,
        age: 28,
        position: "záložník",
        price: 40000
      },
      {
        id: 39,
        name: "Lukáš Hejda",
        teamId: 4,
        age: 25,
        position: "záložník",
        price: 25000
      },
      {
        id: 40,
        name: "Adrián Šemík",
        teamId: 4,
        age: 23,
        position: "útočník",
        price: 20000
      },
      {
        id: 41,
        name: "Jakub Řezníček",
        teamId: 4,
        age: 22,
        position: "útočník",
        price: 25000
      },
      {
        id: 42,
        name: "Michal Krmenčík",
        teamId: 4,
        age: 28,
        position: "útočník",
        price: 50000
      }
    ];
  
  const teams = [
    {
      id: 1,
      name: "AC Sparta Praha",
      city: "Praha",
      stadium: "Generali Arena",
      capacity: 19260,
      sponsor: "ePojisteni.cz"
    },
    {
      id: 2,
      name: "SK Slavia Praha",
      city: "Praha",
      stadium: "Eden Aréna",
      capacity: 21000,
      sponsor: "Europ Assistance"
    },
    {
      id: 3,
      name: "FC Baník Ostrava",
      city: "Ostrava",
      stadium: "Městský stadion",
      capacity: 15250,
      sponsor: "OKD"
    },
    {
      id: 4,
      name: "FC Viktoria Plzeň",
      city: "Plzeň",
      stadium: "Doosan Arena",
      capacity: 11246,
      sponsor: "Škoda Transportation"
    }
  ]

// TODO: Replace the following with your app's Firebase project configuration
// See: https://firebase.google.com/docs/web/learn-more#config-object
const firebaseConfig = {
    // ...
    // The value of `databaseURL` depends on the location of the database
    databaseURL: "https://vfm2023-e59bb-default-rtdb.europe-west1.firebasedatabase.app/",
};

// Initialize Firebase
const app = initializeApp(firebaseConfig);

// Initialize Realtime Database and get a reference to the service
const database = getDatabase(app);

// get reference to data in the database
const dbRef = ref(database);
const teamsRef = ref(database, 'teams');
const playersRef = ref(database, 'players');
// set(playersRef, players)
export const getPlayersFromFirebase = async (teamId) => {
    const playersQuery = query(playersRef, orderByChild("teamId"), equalTo(teamId));
    return new Promise((resolve, reject) => {
        onValue(playersQuery, (snapshot) => {
            const data = snapshot.val();
            const players = Array.isArray(data) ? data : Object.values(data);
            resolve(players);
        }, (error) => {
            reject(error);
        });
    });
};

export const getTeamById = async (teamId) => {
    const teamSnapshot = await get(child(dbRef, `teams/${teamId - 1}`));
    if (teamSnapshot.exists()) {
        const team = teamSnapshot.val();
        return team;
        console.log(team);
    } else {
        console.log("Tým s ID", teamId, "neexistuje v databázi.");
    }
}

export const getTeamsFromFirebase = async () => {

    return new Promise((resolve, reject) => {
        onValue(teamsRef, (snapshot) => {
            const teams = snapshot.val();
            resolve(teams);
        }, (error) => {
            reject(error);
        });
    });
};
